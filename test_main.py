...
def check_url_in_cache(url):
    ...
    print(f"{cache_redirector_url}?{url_encoded}")
...
def test_url_caching(empty_deployment):
        ...
        "checkouts": [
            {
                "id": "_:1",
                "origin": "_",
                "log_url": "https://github.com/kernelci/kcidb/blob/main/.github/workflows/deploy.yml?padding=4821",
                "patchset_files": [
                    {
                        "name": "file",
                        "url": "https://github.com/kernelci/kcidb/blob/main/doc/installation.md?padding=1505"
                    }
                ],
            },
        ],
        "builds": [
            {
                ...
                "input_files": [
                    {
                        "name": "kernel_image1",
                        "url": "https://github.com/kernelci/kcidb/blob/main/doc/_index.md?padding=7230"
                    }
                ],
                "log_url": "https://github.com/kernelci/kcidb/blob/main/.pylintrc?padding=247",
                "config_url": "https://github.com/kernelci/kcidb/blob/main/.gitignore?padding=547",
                "output_files": [
                    {
                        "name": "kernel_image",
                        "url": "https://github.com/kernelci/kcidb/blob/main/Dockerfile?padding=200"
                    },
                    {
                        "name": "kernel",
        ...
        "tests": [
            {
                ...
                "id": "_:1",
                "origin": "_",
                "log_url": "https://cdn.kernel.org/pub/linux/kernel/v6.x/linux-6.4.11.tar.xz",
            },
            {
                "build_id": "kernelci:kernelci.org:64147283e6021132258c86c0",
                "id": "_:1",
                ...
                "output_files": [
                    {
                        ...
                        "url": "https://github.com/kernelci/kcidb/blob/main/setup.py?padding=4673"
                    },
                    {
                        "name": "x86_64_4_IOMMU_boot_test_dmesg.log",
                        "url": "https://github.com/kernelci/kcidb/blob/main/requirements.txt?padding=1649"
                    },
                ],
                "environment": {
                    "comment": "meson-s905d-p230 in lab-baylibre",
                    "misc": {
                        "rootfs_url": "http://storage.kernelci/buildroot-baseline/20230120.0/armel/rootfs.cpio.gz"
                    }
                },
            },
        ],
    ...
    deadline_time = current_time + 120

    # URLs to be check, as they should be cached
    urls_messages = [
        "https://github.com/kernelci/kcidb/blob/main/setup.py?padding=4673",
        "https://github.com/kernelci/kcidb/blob/main/requirements.txt?padding=1649",
        "https://github.com/kernelci/kcidb/blob/main/.gitignore?padding=547",
        "https://github.com/kernelci/kcidb/blob/main/.pylintrc?padding=247",
        "https://github.com/kernelci/kcidb/blob/main/doc/_index.md?padding=7230",
        "https://github.com/kernelci/kcidb/blob/main/.github/workflows/deploy.yml?padding=4821",
        "https://github.com/kernelci/kcidb/blob/main/doc/installation.md?padding=1505"
    ]

    # Retry checking URLs in the cache for a minute
    retry_interval = 5  # seconds

    for url in urls_messages:
        while True:
    ...
    urls_not_expected = [
        ...
        # Invalid url
        'https://kernelcdn.kernel.org/pub/linux/',

        # Larger-than-maximum size URL
        "https://cdn.kernel.org/pub/linux/kernel/v6.x/linux-6.4.11.tar.xz",

        # Wrong hash URL
        "https://github.com/kernelci/kcidb/blob/main/Dockerfile?padding=200",

        # URL from a wrong field
        "http://storage.kernelci/buildroot-baseline/20230120.0/armel/rootfs.cpio.gz",
    ]

    for url_not_expected in urls_not_expected:
        if check_url_in_cache(url_not_expected):
            raise AssertionError(f"Unexpected URL '{url_not_expected}' found in the cache")
