...
class KCIDBDBMainFunctionsTestCase(kcidb.unittest.TestCase):
    ...
    def test_load_main(self):
        ...
        driver_source = textwrap.dedent("""
            from unittest.mock import patch, Mock
            from kcidb_io.schema import V5
            client = Mock()
            client.get_schema = Mock(return_value=((1, 0), V5))
...
class KCIDBDBClientTestCase(kcidb.unittest.TestCase):
    ...
    def test_upgrade(self):
        ...
        db_client.init((4, 0))
        self.assertEqual(db_client.get_schema(), ((4, 0), kcidb.io.schema.V4))
        ...
        v5_oo_data = kcidb.misc.merge_dicts(
            v4_oo_data,
            bug=[
                {
                    "culprit_code": True,
        ...
        db_client.load(v4_data)
        self.assertEqual(db_client.dump(), v4_data)
        self.assertEqual(db_client.oo_query(kcidb.orm.Pattern.parse(">*#")),
                         v4_oo_data)
        with self.assertRaises(AssertionError):
            db_client.load(v5_data)
        db_client.upgrade((4, 1))
        self.assertEqual(db_client.get_schema(), ((4, 1), kcidb.io.schema.V5))
        self.assertEqual(db_client.oo_query(kcidb.orm.Pattern.parse(">*#")),
                         v4_oo_data)
        with self.assertRaises(AssertionError):
            db_client.load(v4_data)
        upgraded_v4_data = kcidb.io.schema.V5.upgrade(v4_data)
        self.assertEqual(db_client.dump(), upgraded_v4_data)
        db_client.load(v5_data)
        merged_data = {**upgraded_v4_data, **v5_data}
